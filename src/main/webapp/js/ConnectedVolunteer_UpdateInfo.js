async function GetUserInfo() {
    ...
    try {
        const response = await fetch('/E-199/ConnectedVolunteer_GetInfo', {
            method: 'GET',
        ...
        if (result.success) {
            ...
            document.getElementById("categoryDisplay").textContent = result.data.volunteer_type;
            document.getElementById("weight").textContent = result.data.weight;
            document.getElementById("height").textContent = result.data.height;
            return result;
        } else {
...
async function UpdateUserInfo(event) {
    ...
    try {
        const response = await fetch('/E-199/ConnectedVolunteer_UpdateInfo', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
