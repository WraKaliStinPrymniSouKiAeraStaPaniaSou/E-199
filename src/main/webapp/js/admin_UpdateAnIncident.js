...
function LoadIncidentDetails() {
    const selected_incident_id = document.getElementById("RunningIncidents").value;
...
async function UpdateIncident(event) {
    ...
    try {
        const response = await fetch('/E-199/admin_UpdateAnIncident', {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            submit_message.textContent = "The incident was updated successfully!";
            submit_message.style.color = "green";
        }
    } catch (error) {
        console.error("Error while updating the incident:", error.message);
...
document.getElementById("RunningIncidents").addEventListener("change", LoadIncidentDetails);
document.getElementById("form").addEventListener("submit", UpdateIncident);