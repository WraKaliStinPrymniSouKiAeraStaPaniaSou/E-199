...
function print_form(event) {
    ...
    const form_data = {
        ...
        category: document.getElementById('category') ? document.getElementById('category').value : null,
...
async function checkUsername() {
    ...
    if (username) {
        try {
            ...
            const response = await fetch(`/E-199/Volunteer_CheckName?username=` + encodeURIComponent(username));
...
async function checkEmail() {
    ...
    if (email) {
        // Send GET requerst to server.
        const response = await fetch('/E-199/Volunteer_CheckEmail?email=' + encodeURIComponent(email)) ;
...
async function checkPhone() {
    ...
    if (telephone) {
        // Send GET requerst to server.
        const response = await fetch('/E-199/Volunteer_CheckTelephone?telephone=' + encodeURIComponent(telephone));
        
        if (!response.ok) {
            console.error("Error in response");
            check_telephone_message.textContent = "Error! Type the telephone again!";
...
async function register() {
    ...
    const form_data = {
        ...
        volunteer_type: document.getElementById('category').value,
    ...
    try {
        const response = await fetch('/E-199/VolunteerRegistration', {
            method: 'POST',
